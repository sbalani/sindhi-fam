...
import { draftPersonName, interpretFamilyStory, VOICE_LANGUAGES } from './familyInterpreter.js'
...
const TEXT = {
  en: {
    ...
    reviewTitle: 'Check each person and relationship', relation: 'Relationship to narrator', name: 'Name', age: 'Age', location: 'Current / last known location', birth: 'Birth location', evidence: 'Why Vansh mapped this',
  ...
  es: {
    ...
    reviewTitle: 'Comprueba cada persona y relación', relation: 'Relación con quien habla', name: 'Nombre', age: 'Edad', location: 'Ubicación actual / última conocida', birth: 'Lugar de nacimiento', evidence: 'Por qué Vansh hizo esta conexión',
  ...
  sd: {
    ...
    reviewTitle: 'هر شخص ۽ لاڳاپو چيڪ ڪريو', relation: 'ڳالهائيندڙ سان لاڳاپو', name: 'نالو', age: 'عمر', location: 'هاڻوڪي / آخري ڄاڻايل جاءِ', birth: 'ڄمڻ جي جاءِ', evidence: 'Vansh هي لاڳاپو ڇو ٺاهيو',
...
function RoughTree({ draft, copy, onSelect }) {
  ...
  return <div className="voice-rough-map">
    <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      {links.map(r => pos[r.from] && pos[r.to] ? <line key={r.id} x1={pos[r.from][0]} y1={pos[r.from][1]} x2={pos[r.to][0]} y2={pos[r.to][1]} className={r.type} /> : null)}
    </svg>
...
export default function VoiceFamilyImport({ people, relationships, onCommit, onOpenTree }) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = () => useBlob(new Blob(chunks, { type: recorder.mimeType || 'audio/webm' }))
  ...
  const reset = () => { setStep('capture'); setTranscript(''); setDraft(null); setReviewIndex(0); setError(''); useBlob(null) }
    ...
    {step === 'capture' && <>
      ...
      <div className="voice-grid">
        <section className="panel voice-record-panel">
          ...
          <div className="voice-audio-row"><input ref={fileRef} hidden type="file" accept="audio/*" onChange={e => e.target.files?.[0] && useBlob(e.target.files[0])}/><button className="secondary" onClick={() => fileRef.current?.click()}><Upload size={15}/>{copy.upload}</button>{audioUrl && <audio controls src={audioUrl}/>}</div>
    ...
    {step === 'review' && draft && <>
      ...
      {!reviewDone && current ? <section className="panel voice-review-card">
        ...
        <div className="voice-review-form">
          <label className="wide">{copy.relation}<div className="voice-relation"><Network size={14}/><strong>{current.relationToNarrator}</strong></div></label>
