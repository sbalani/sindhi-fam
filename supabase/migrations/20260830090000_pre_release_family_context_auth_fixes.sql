...
-- ---------------------------------------------------------------------------
-- Normalize public -> auth.users foreign keys.
-- Existing Vansh projects predate the canonical baseline, so CREATE TABLE IF
-- NOT EXISTS could not repair older NO ACTION foreign keys. Those legacy FKs
-- can make Supabase Auth show "Database error deleting user".
-- ---------------------------------------------------------------------------
do $$
declare
  target record;
...
alter table public.family_members
  add constraint family_members_owner_auth_fkey foreign key (owner_id) references auth.users(id) on delete cascade,
...
alter table public.relationships
  add constraint relationships_owner_auth_fkey foreign key (owner_id) references auth.users(id) on delete cascade,
...
alter table public.family_invitations
  add constraint family_invitation_graph_owner_auth_fkey foreign key (graph_owner_id) references auth.users(id) on delete cascade,
  add constraint family_invitation_inviter_auth_fkey foreign key (inviter_id) references auth.users(id) on delete cascade,
